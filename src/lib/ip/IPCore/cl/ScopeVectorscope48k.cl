                           ...
                           uint imageHeight)
{
    ...
    // Fixed-point RGB (x1024)
    uint rVal = (uint)(r * 1024.0f);
    uint gVal = (uint)(g * 1024.0f);
    uint bVal = (uint)(b * 1024.0f);

    // Scatter to global 2D grid: layout [crBin * VSCOPE_BIN + cbBin] * CHANNEL
...
// Launch with globalSize = (VSCOPE_BIN, VSCOPE_BIN).
__kernel
                                ...
                                float imgSize)
{
    ...
    // Convert from fixed-point (x1024) back to float and normalize by pixel count
